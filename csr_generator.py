...
base_pkt = (
    ...
    / GTPV2CreateSessionRequest(
        IE_list=[
            IE_IMSI(ietype=1, length=8, CR_flag=0, instance=0, IMSI="001011234567895"),
            ...
            IE_RAT(ietype=82, length=1, CR_flag=0, instance=0, RAT_type=3),
            IE_FTEID(
                ietype=87,
                length=9,
                CR_flag=0,
...
for i in range(run):
    ...
    data_iters[i]['imsi'] = str('001011234567895')
...
def composeCSReq(base_pkt,srcnet,data_iter):
    ...
    pkt.seq = next(seq)
    pkt.IE_list[0].fields["IMSI"] = str(data_iter['imsi'])
